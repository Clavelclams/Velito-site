 ...
 *  - Les règles métier (égalité interdite, match incomplet…) vivent dans
 *    lib/bracket.ts (pur, testé) — les actions ne font qu'orchestrer.
 *  - Toute action sensible écrit dans arena_logs (traçabilité subventions + audit).
 *
 * Server Action = fonction qui s'exécute CÔTÉ SERVEUR, appelable directement
 * depuis un <form action={...}> React. Pas besoin d'écrire des routes API à la
 * main : Next.js génère l'endpoint, on garde le typage de bout en bout.
 */
...
export async function creerTournoi(formData: FormData) {
  ...
  if (maxJoueurs !== null && (maxJoueurs < 2 || maxJoueurs > 128)) {
    throw new Error("max_joueurs doit être entre 2 et 128.");
  }
...
export async function changerStatutTournoi(formData: FormData) {
  ...
  // Transitions autorisées V1 (machine à états simple mais explicite).
  const transitions: Record<string, string[]> = {
    BROUILLON: ["OUVERT", "ANNULE"],
    OUVERT: ["BROUILLON", "ANNULE"], // EN_COURS passe par demarrerTournoi uniquement
    EN_COURS: ["TERMINE", "ANNULE"],
    TERMINE: [],
    ANNULE: [],
  };
  if (!transitions[tournoi.statut]?.includes(nouveau)) {
...
export async function ajouterJoueurStaff(formData: FormData) {
  ...
  // 23505 = violation d'unicité → le joueur est déjà inscrit, pas une vraie erreur.
...
export async function saisirScore(formData: FormData) {
  ...
  if (!Number.isInteger(scoreJ1) || !Number.isInteger(scoreJ2) || scoreJ1 < 0 || scoreJ2 < 0) {
...
export async function validerScore(formData: FormData) {
  ...
  if (m.statut !== "TERMINE") {
    throw new Error("Le score doit d'abord être saisi (statut TERMINE) avant validation.");
  }
  ...
  if (prog.parent) {
    const parent = matchs.find(
      (x) => x.round === prog.parent!.round && x.position === prog.parent!.position
    );
    if (parent) {
      await db
        .from("arena_matchs")
